    ...
    while(complete == false){
      ...
      for(let p = 1; p < All7thStudentData.length; p++){
        ...
        if(lowRSP){
          ...
          failed = assignPeriods(advancedMathPeriods_7, allPeriodCounters_7, allPeriods_7)
        }
  
        else{
          failed = assignPeriods(nonAdvancedMathPeriods_7, allPeriodCounters_7, allPeriods_7)
        }
...
function assignPeriods(periodList, periodCounters, allPeriodList){
  j = 0;
  for(let i = 1; i < 6; i++){
    ...
    if(studentSchedule.indexOf(periodList[i-1][period]) > -1){
        ...
        while(true){
          ...
          if(newSchedule.length == 5){
            for(let y = 0; y < newSchedule.length; y++){
              periodCounters[y][allPeriodList[y].indexOf(newSchedule[y])]++;
            }
            resolved = true;
      ...
      periodCounters[i-1][allPeriodList[i-1].indexOf(periodList[i-1][period])]++
      j = 0;
    }
  }
  return failed;
